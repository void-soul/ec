...
proto.select = function* (table, options) {
  options = options || {};
  const sql = this._selectColumns(table, options.columns) + this._where(options.where) + this._orders(options.orders) + this._limit(options.limit, options.offset);
  debug('select(%j, %j) \n=> %j', table, options, sql);
  return this.mock === true ? sql : yield this.query(sql);
};
...
SELECT 10 ,'ab',30 FROM DUAL WHERE NOT EXISTS
    (SELECT 1 FROM table2 where a = b)
options: [{ row: { name: 'fengmk21' }, where: { id: 1 } }]
 */
proto.insertIF = function* (table, options, columns) {
  ...
  for (let i = 0; i < options.length; i++) {
    const values = [];
    const param = [];
    const row = options[i].row;
    for (let j = 0; j < columns.length; j++) {
      const vl = row[columns[j]];
      values.push(vl === undefined ? null : vl);
    }
    param.push(values);
    param.push(table);

    strs.push(this.format('SELECT ? FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM ?? ', param) + this._where(options[i].where) + ')');
  }

  const sql = this.format('INSERT INTO ??(??) ', params) + this.format(strs.join(' UNION ', params));
  debug('insertIF(%j, %j) \n=> %j', table, options, sql);
  return this.mock === true ? sql : yield this.query(sql);
};
proto.insert = function* (table, rows, options) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (let j = 0; j < options.columns.length; j++) {
      const vl = row[options.columns[j]];
      values.push(vl === undefined ? null : vl);
    }
    strs.push('(?)');
    params.push(values);
...
proto.replace = function* (table, rows, options) {
  options = options || {};
  let firstObj;
  if (Array.isArray(rows)) {
    firstObj = rows[0];
  ...
  if (!options.ids) {
    throw new Error('Can not auto detect update condition, please set options.ids');
  }
  const sets = [];
  const values = [];
  for (let i = 0; i < options.columns.length; i++) {
    const column = options.columns[i];
    if (options.ids.includes(column)) {
      continue;
    }
    sets.push('?? = VALUES(??)');
    values.push(column);
  ...
  const sql = this.format('INSERT INTO ??(??) VALUES' + strs.join(', '), params) + ' ON DUPLICATE KEY UPDATE ' + this.format(sets.join(', '), values);
  debug('insert(%j, %j, %j) \n=> %j', table, rows, options, sql);
  return this.mock === true ? sql : yield this.query(sql);
};
...
proto.updateUnSafe = function* (table, row, options) {
  options = options || {};
  if (!options.columns) {
    options.columns = Object.keys(row);
  }
...
proto.delete = function* (table, where) {
  const sql = this.format('DELETE FROM ??', [table]) + this._where(where);
  debug('delete(%j, %j) \n=> %j', table, where, sql);
  return this.mock === true ? sql : yield this.query(sql);
};
