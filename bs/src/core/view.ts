import {imageManager, devToolSwitch, uriParse, buildWebPreferences, confirm} from '../util';
...
import {UrlInfo, ViewOption, ViewNetState, ViewMode, CloseMode, ViewPoint, ContextMode} from 'plugin-line';
import {DEF_TITLE, DEF_VIEW_POINT} from '../util/global';
...
export default class JingView {
  ...
  constructor (option: ViewOption) {
    ...
    this.pluginName = option.pluginName;
    this.contextMode = option.contextMode;
    ...
    view.webContents
      ...
      .on('dom-ready', () => {
        // TODO: 注入脚本、样式
        jingApp.inject(view.webContents);
      })
      ...
      .on('context-menu', (_event: Event, params: ContextMenuParams) => {
        ...
        if (this.contextMode === 'Auto') {
          jingApp.autoContextMenu(JingWindow.fromId(this.windowId), JingView.fromId(this.id), params);
        }
      })
  ...
  async loadURL(url?: string, options?: LoadURLOptions) {
    ...
    if (url) {
      ...
      try {
        await webContents.fromId(this.webContentId).loadURL(this.url.fullUrl, options);
      } catch (error) {
        log.error(error);
      }
    }
  ...
  /** 右键菜单 */
  customContextMenu(...args: any[]) {
    jingApp.customContextMenu(JingWindow.fromId(this.windowId), JingView.fromId(this.id), ...args);
  }
  info() {
    return {
      id: this.id,
