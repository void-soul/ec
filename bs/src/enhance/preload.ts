...
console.info(`preload! window=${ windowid }, view=${ viewid }`);
...
window.brage = {
  ...
  getWindow(_windowid?: number) {
    _windowid = _windowid || windowid as number || 0;
  ...
  getView(_viewid?: number) {
    _viewid = _viewid || viewid as number || 0;
    if (!viewCache[_viewid]) {
      viewCache[_viewid] = {
        ...
        customContextMenu: (...args) => ipcRenderer.send('on=jing=view', _viewid, 'customContextMenu', ...args),
      };
    }
