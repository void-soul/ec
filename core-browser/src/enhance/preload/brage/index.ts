...
const {windowid, viewid} = ipcRenderer.sendSync(MY_INFO);
window.brage = {
  windowid,
  viewid,
  activeid() {
  ...
  invoke: async (channel: string, ...args: any[]) => {
    return ipcRenderer.invoke(INVOKE_BACK, channel, ...args);
  },

  excute: (channel: string, ...args: any[]) => {
    return ipcRenderer.sendSync(EXCUTE_BACK, channel, ...args);
  },
